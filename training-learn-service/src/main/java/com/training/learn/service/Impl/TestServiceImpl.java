...
public class TestServiceImpl implements TestService {
    ...
    public MsgRespond creatTest(TestReq req) throws ParseException {
        ...
        //判断是否发布，默认未不发布
        if (req.getIsRelease() == null){
            req.setIsRelease(0);
        }
        //判断如果为已发布，则设置当前为发布时间
        if (req.getIsRelease() == 1){
            if (req.getCreate_datetime() == null){
                req.setCreate_datetime(getNowTime());
            }
        }
     ...
     */
    @Override
    public DataRespond getQuestion(int test_id, String auth) throws ParseException {
        //判断测试的存在性
        Test test = testMapper.getTestById(test_id);
        if (test == null){
            return new DataFailRespond("该测试不存在！");
        }
        //判断数据库中是否有该测试的试题，有则说明已经发布
        List<QuestionTable> judgeQuestionExit = questionMapper.getQuestionByTestId(test_id);
        if (judgeQuestionExit.isEmpty() && Objects.equals(auth,"teacher")){
                List<CacheReq.Question> questions = questionCache.getCache(test_id);
                if (questions.isEmpty()){
                    return new DataFailRespond("未有编写记录");
                }else {
                    return new DataSuccessRespond("已成功获取试题列表",questions);
                }
        }
        List<Integer> allQuestionIdList = questionMapper.getIdByTestId(test_id);
        //获取缓存数据
        List<TeaQuestionResult.Question> questionList = questionCache.getQuestion(test_id);
        //判断请求用户为普通员工时
        if (Objects.equals(auth,"none")){
           SimpleDateFormat si = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
           //判断当前时间是否早于起始时间时
           if(si.parse(test.getStart_datetime()).getTime()>System.currentTimeMillis()){
               return new DataFailRespond("考试尚未开始，无法查看试卷题目");
           }
           //判断当前时间是否晚于起始时间
           if (si.parse(test.getEnd_datetime()).getTime()<System.currentTimeMillis()){
               return new DataFailRespond("考试已经结束，无法查看试卷题目");
           }
           //获取测试题目
           List<StuQuestionResult.Question> stulist = new ArrayList<>();
           for(Integer i:allQuestionIdList){
               ...
               Map<String,String> options = JSON.parseObject(optionsTable.getOption(), Map.class);
               ...
               stulist.add(question);
           }
           StuQuestionResult stuQuestionResult = new StuQuestionResult(test.getTest_title(),test.getStart_datetime(),test.getEnd_datetime(),stulist);
        ...
        if (Objects.equals(auth,"teacher")){
            ...
            if (questionList.isEmpty()){
                ...
                //将试题保存至缓存
                questionCache.saveQuestion(test_id,questionList);
            }
    ...
    public DataRespond TeaGetAllScore(int test_id, int page_size, int offset) {
        ...
        for(ScoreTable scoreTable :scoreTables){
            ...
            ScoreResult scoreResult = new ScoreResult(user_info,score);
            scoreResultList.add(scoreResult);
        }
        //实现分页
        int endIndx = Math.min(offset + page_size,scoreResultList.size());
        List<ScoreResult> results = scoreResultList.subList(offset,endIndx);
    ...
    public DataRespond getLessonAllScore(int lesson_id, int page_size, int offset) {
        ...
        List<LessonScoreResult> result = list.subList(offset,endIndx);
